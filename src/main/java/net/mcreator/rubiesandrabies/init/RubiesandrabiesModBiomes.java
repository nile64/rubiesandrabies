...
public class RubiesandrabiesModBiomes {
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					addParameterPoint(parameters, new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(0f, 1f), Climate.Parameter.span(0.5f, 1f), Climate.Parameter.span(0.3f, 1f), Climate.Parameter.span(0.5f, 1f),
	...
	private static SurfaceRules.RuleSource injectNetherSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(
				anySurfaceRule(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("rubiesandrabies", "racism")), Blocks.GRASS_BLOCK.defaultBlockState(), Blocks.DIRT.defaultBlockState(), Blocks.GRAVEL.defaultBlockState()));
		if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
			customSurfaceRules.addAll(sequenceRuleSource.sequence());
			return SurfaceRules.sequence(customSurfaceRules.toArray(SurfaceRules.RuleSource[]::new));
		} else {
