...
@EventBusSubscriber
public class DimensionOfPeaceTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(RubiesandrabiesModBlocks.DIMENSION_OF_PEACE_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("rubiesandrabies:dimension_of_peace_portal"), poiType);
	...
	public DimensionOfPeaceTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = RubiesandrabiesModBlocks.DIMENSION_OF_PEACE_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
